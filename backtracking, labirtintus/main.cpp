...
bool sajttaken(int tomb[100][100], int n, int m){
for(int i=0; i<n; i++){
    for(int j=0; j<m; j++){
        if(tomb[i][j]==4){
            return false;
...

{
    ...
    if(tomb[xpos +1][ypos] == 3 || tomb[xpos -1][ypos] == 3 || tomb[xpos][ypos +1] == 3 || tomb[xpos][ypos-1] == 3 ){
        return false;
    }
